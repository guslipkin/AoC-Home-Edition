...
df <-
  data.frame(matrix(NA, nrow = (rowCount + 2), ncol = (colCount + 2)))
...
while (!fullFlag) {
  ...
  for (r in rowSeq) {
    for (c in colSeq) {
      if (is.na(df[r, c])) {
        boxSize <- as.numeric(numberDF[r, c])
        while (TRUE) {
          colorNum <- floor(runif(1, min = 1, max = length(colors) + 1))
          color <- colors[colorNum]
          numSpot <- match(boxSize, colorSize[[colorNum]])
          numSpot <- numSpot[1]
          if (!is.na(numSpot) & squaresRemaining[[colorNum]] != 0)
            break
        }
  ...
  if (!(FALSE %in% (sameCheck == squaresRemaining)))
    ...
    for (c in colSeq) {
      ...
      if (is.na(df[r, c])) {
        ...
        while (TRUE) {
          colorNum <- floor(runif(1, min = 1, max = length(colors) + 1))
          color <- colors[colorNum]
          numSpot[1] <- match(numberDF[r, c], colorSize[[colorNum]])
          boxSize <- colorSize[[colorNum]][numSpot[1]]
          if (!is.na(numSpot[1]) & squaresRemaining[[colorNum]] != 0)
            break
        }
        colorSize[[colorNum]] <- colorSize[[colorNum]][-numSpot[1]]
        ...
        if ((r + boxSize) <= (nrow(df) - 1) & (c + boxSize) <= (ncol(df) - 1) & squaresRemaining[[colorNum]] > 0) {
          temp <- as.data.frame(df[(r - 1):(r + boxSize), (c - 1):(c + boxSize)])
          temp[2:(nrow(temp) - 1), 2:(ncol(temp) - 1)] <- NA
          temp[temp == "x"] <- NA
          if (nrow(temp) == 1)
            temp[1, 1] <- NA
          ...
          if (!(FALSE %in% is.na(as.data.frame(df[r:(r + boxSize - 1), c:(c + boxSize - 1)]))) & !is.element(color, unlist(temp))) {
            validFlag <- TRUE
            squaresRemaining[[colorNum]] <-
              squaresRemaining[[colorNum]] - 1
            df[r:(r + boxSize - 1), c:(c + boxSize - 1)] <- color
          }
        }
        
        # if (!is.na(squaresRemaining[colorNum]) & squaresRemaining[[colorNum]] == 0) {
