  ...
  for(r in rowSeq) {
    for(c in colSeq) {
      ...
      while(TRUE) {
        ...
        if(!is.na(match(numberDF[r,c], colorSize[[colorNum]])) & !is.na(colorSize[[colorNum]] | is.na(colorSize[[colorNum]])))
          break
      }
      ...
      leftovers[1] <- squaresRemaining[[colorNum]] - 1
      
      #if((r + boxSize) <= (nrow(df) - 1) & (c + boxSize) <= (ncol(df) - 1)) {
      if((r + boxSize) <= (nrow(df) - 1) & (c + boxSize) <= (ncol(df) - 1) & as.numeric(leftovers[1]) >= 0) {
        ...
        if(!(FALSE %in% is.na(as.data.frame(df[r:(r+boxSize), c:(c+boxSize)]))) & !is.element(color, unlist(temp))) {
            validFlag <- TRUE
            squaresRemaining[[colorNum]] <- leftovers[1]
